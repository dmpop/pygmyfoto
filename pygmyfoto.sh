...
if [ ! -f pygmyfoto.sqlite ];
	then
	sqlite3 pygmyfoto.sqlite "CREATE TABLE photos (id INTEGER PRIMARY KEY UNIQUE NOT NULL, title VARCHAR(512), description VARCHAR(1024), tags VARCHAR(256), exif VARCHAR(1024), dt DATE, published VARCHAR(1), count INTEGER DEFAULT 0);"
fi
...
sqlite3 pygmyfoto.sqlite "INSERT INTO photos (title, description, tags, exif, dt, published) VALUES ('$TITLE', '$DESCRIPTION', '$TAGS', '$EXIF', '$DATE', '1');"
